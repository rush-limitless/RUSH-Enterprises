  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowForm(true)} size="sm" className="bg-primary hover:bg-primary/90 gap-1.5">
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={() => setShowForm(false)}>
          <Card className="w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <CardContent className="space-y-3">
              <div><Label className="text-xs">Name</Label><Input value={form.name} onChange={e => set("name", e.target.value)} className="mt-1 h-8 text-sm" /></div>
              <div><Label className="text-xs">Category</Label><Input value={form.category} onChange={e => set("category", e.target.value)} className="mt-1 h-8 text-sm" /></div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label className="text-xs">Price (FCFA)</Label><Input type="number" value={form.price} onChange={e => set("price", e.target.value)} className="mt-1 h-8 text-sm" /></div>
                <div><Label className="text-xs">Stock</Label><Input type="number" value={form.stock} onChange={e => set("stock", e.target.value)} className="mt-1 h-8 text-sm" /></div>
              </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-[11px] text-right">Stock</TableHead>
            ...
            <TableBody>
              {filtered.map(p => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge variant="secondary" className="text-[10px]">{p.type === "digital" ? "Digital" : "Physical"}</Badge>
                  </TableCell>
                  <TableCell className="text-xs text-right">{p.price.toLocaleString("fr-FR")} FCFA</TableCell>
                  <TableCell className="text-right">
                    <span className={`text-xs font-semibold ${p.minStock && p.stock <= p.minStock ? "text-red-500" : ""}`}>{p.stock}</span>
                  </TableCell>
                  <TableCell>
